...
public class RopeRelayRenderer extends SafeBlockEntityRenderer<RopeRelayBlockEntity> {

    public RopeRelayRenderer(final BlockEntityRendererProvider.Context context) {
    ...
    @Override
    public boolean shouldRenderOffScreen(final RopeRelayBlockEntity be) {
        return true;
    }

    @Override
    public boolean shouldRender(final RopeRelayBlockEntity pBlockEntity, final Vec3 pCameraPos) {
        return true;
    }

    protected void renderSafe(final RopeRelayBlockEntity be, final float partialTicks, final PoseStack ms, final MultiBufferSource buffer, final int light, final int overlay) {
        FilteringRenderer.renderOnBlockEntity(be, partialTicks, ms, buffer, light, overlay);
        this.renderComponents(be, partialTicks, ms, buffer, light, overlay);
    }

    protected void renderComponents(final RopeRelayBlockEntity be, final float partialTicks, final PoseStack ms, final MultiBufferSource buffer, final int light, final int overlay) {
        ms.pushPose();
