 ...
 * {@code assets/setsail/lang/default/*.json}, and only the translation <em>keys</em>
 * appear in code (see {@link com.theGuruGuru.setsail.Lang}).
 *
 ...
 * into that same provider via {@\link CreateSetSail#registerLangPartials()}, so the partials
 ...
 */
public class CreateSetSailLangMerger {

    private static final Gson GSON = new Gson();

    /** Partial file names under {@code assets/setsail/lang/default/}, without ".json". */
    ...
    public static void mergeInto(BiConsumer<String, String> consumer) {
        for (String partial : PARTIALS) {
            String path = "assets/" + CreateSetSail.ID + "/lang/default/" + partial + ".json";
            try (InputStream in = CreateSetSailLangMerger.class.getClassLoader().getResourceAsStream(path)) {
                if (in == null) {
                    CreateSetSail.LOGGER.error("Lang partial not found on the classpath: {}", path);
                    continue;
                }
