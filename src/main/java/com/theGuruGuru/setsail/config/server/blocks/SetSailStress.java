package com.theGuruGuru.setsail.config.server.blocks;
...
public class SetSailStress extends CStress {
    ...
    public void registerAll(final ModConfigSpec.Builder builder) {
        builder.comment(new String[]{".", SetSailStress.Comments.su, SetSailStress.Comments.impact}).push("impact");
        DEFAULT_IMPACTS.forEach((id, value) -> this.impacts.put(id, builder.define(id.getPath(), value)));
        builder.pop();
        builder.comment(new String[]{".", SetSailStress.Comments.su, SetSailStress.Comments.capacity}).push("capacity");
    ...
    private static void assertFromSimulated(final BlockBuilder<?, ?> builder) {
        if (!builder.getOwner().getModid().equals("setsail")) {
            throw new IllegalStateException("Non-SetSail blocks cannot be added to Simulated's config.");
        }
    }
